...
MainWindow::MainWindow(const std::string& name, const std::string& version, QWidget* parent) : QMainWindow(parent), afs(NULL), thread(NULL), ui(new Ui::MainWindow)
{
...
void MainWindow::startExporting(const QList<int>& list, const std::string& path) // TODO -> lock function on MainWindow during thread execution
{
	thread = new Unpacker(afs, list, path);

	connect(thread, SIGNAL(error(QString, bool)), this, SLOT(error(QString, bool)));
	connect(thread, SIGNAL(done()), this, SLOT(done()));
	connect(thread, SIGNAL(abort()), this, SLOT(abort()));

	thread->start(); // call run() function
}
...
{
	QList<int> list = getSelectedRows();
